...
const WeddingAnnouncementEdit = (props) => {
	...
	useEffect(
		() => props.get(`wedding-announcements/${id}`, setWeddingAnnouncement),
		[]
	)
	...
	const onSubmit = (e) => {
		...
		Axios.post(`/api/wedding-announcements/${id}`, formData)
			.then((res) => {
				...
				setTimeout(
					() => router.push(`/wedding-announcement/show/${res.data.data.id}`),
					500
				)
			})
			.catch((err) => {
	...
	const onDelete = () => {
		...
		Axios.delete(`api/wedding-announcements/${id}`)
			.then((res) => {
				// Remove loader
				...
				<center>
					...
					<form onSubmit={onSubmit}>
							...
							<div className="col-lg-4 col-sm-12">
								<FilePond
									...
									server={{
										url: `/api/filepond`,
										process: {
											url: "/wedding-announcement-poster",
											onload: (res) => setPoster(res),
											onerror: (err) => console.log(err.response.data),
										},
										revert: {
											url: `/wedding-announcement-poster/${poster.substr(27)}`,
											onload: (res) => {
						...
						<input
							...
							placeholder={weddingAnnouncement.name}
						...
						<textarea
							...
							name="description"
							className="form-control"
							placeholder={weddingAnnouncement.eulogy}
							cols="30"
							rows="5"
						...
						<MyLink
							linkTo={`/wedding-announcement/show/${id}`}
...
export default WeddingAnnouncementEdit
